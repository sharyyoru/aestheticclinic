...
function getRecommendation(row: {
  ...
  if (row.is_duplicate) {
    return { recommendation: "ARCHIVE — paid replacement invoice exists. Mark this invoice as CANCELLED. No resend needed.", action_category: "ARCHIVE" };
  }
  if (row.fully_paid) {
    return { recommendation: "NO ACTION — invoice is fully paid.", action_category: "NO ACTION" };
  }
  if (row.storno_status === "NOT_RESENT") {
    return { recommendation: "RESEND — stornoed but no replacement was sent. Correct the service data (check tariff codes, use TARDOC tariff_type=007) and resend as a new invoice. Amount at risk.", action_category: "RESEND" };
  }
  if (row.storno_status === "RESENT_NOT_PAID") {
    return { recommendation: "FOLLOW UP — replacement invoice was sent but not yet paid. Check with insurer for status.", action_category: "FOLLOW UP" };
  }
  if (row.storno_status === "CANCELLED") {
    return { recommendation: "VERIFY — invoice was cancelled. Check if a replacement was sent manually outside the system.", action_category: "VERIFY" };
  }
  if (!row.sent_to_medidata) {
    return { recommendation: "SEND — invoice has not been submitted to MediData yet. Send it now.", action_category: "SEND" };
  }
  if (row.routing_correct === "NO" && !row.fully_paid) {
    return { recommendation: "RESEND — invoice was sent to the wrong insurer GLN. The routing bug has been fixed. Resend with the correct insurer GLN.", action_category: "RESEND" };
  }
  if (row.final_category === "REJECTED") {
    ...
    if (code && ERROR_CODE_ACTIONS[code]) {
      ...
      return { recommendation: action.action, action_category: action.category };
    }
    return { recommendation: `REVIEW REJECTION — insurer rejected this invoice. Reason: "${reason.substring(0, 150)}". Contact insurer or check error code documentation for specific action.`, action_category: "REVIEW" };
  }
  if (row.final_category === "TRANSMITTED" && !row.fully_paid) {
    if (row.age_days > 90) {
      return { recommendation: "CONTACT INSURER — invoice has been transmitted for 90+ days with no response. The insurer response may have expired on MediData's queue. Contact MediData support or the insurer directly to resend the response.", action_category: "CONTACT INSURER" };
    } else if (row.age_days > 30) {
      return { recommendation: "FOLLOW UP — invoice has been transmitted for 30+ days with no insurer response. Run a manual poll (POST /api/medidata/poll) to check for pending responses. If still no response, contact the insurer directly.", action_category: "FOLLOW UP" };
    } else {
      return { recommendation: "WAIT — invoice was recently transmitted. The insurer has up to 30 days to respond. No action needed yet.", action_category: "WAIT" };
    }
  }
  if (row.final_category === "ACCEPTED") {
    return { recommendation: "NO ACTION — invoice was accepted by the insurer. Payment should follow.", action_category: "NO ACTION" };
  }
  if (row.final_category === "DRAFT") {
    return { recommendation: "SEND — invoice is in draft status. Complete and submit to MediData.", action_category: "SEND" };
  }
  return { recommendation: "REVIEW — check invoice status and submission history for details.", action_category: "REVIEW" };
}

export type MediDataStatusResult = {
  rows: any[];
...
export async function fetchMediDataStatus(
  from: string,
  to: string,
  agingMinDays: number = 0,
): Promise<MediDataStatusResult> {
  const BATCH = 200;
  ...
  if (invoiceIds.length === 0) {
    return { rows: [], summary: {}, agingBuckets: {}, agingAmounts: {}, recommendationSummary: {} };
  }
  ...
  // ── 3. Get submissions (WITHOUT xml_content) ──
  ...
  // ── 3b. Fetch xml_content ONLY for invoices with non-storno submissions ──
  ...
  // ── 5. Duplicate + replacement detection (scoped to patient+insurer pairs) ──
